# ============================================================
# NR-17 | ERGONOMIA POR VISÃO - MVP 04.2 TABLET / PREVIEW LOCAL
# Arquivo único: visão + RULA/REBA + evidências + PDF
# Preview local no tablet + análise assíncrona no Cloud sem retorno de vídeo processado
# ============================================================
...
CAMERA_PROFILES = {
    "Industrial · 1280×720 · 24 FPS": {
        "width": 1280, "height": 720, "fps": 24, "process_long_side": 960, "analysis_fps": 10,
    },
    "Alta precisão · 1920×1080 · 20 FPS": {
        "width": 1920, "height": 1080, "fps": 20, "process_long_side": 960, "analysis_fps": 10,
    },
    "Econômica · 960×540 · 20 FPS": {
        "width": 960, "height": 540, "fps": 20, "process_long_side": 640, "analysis_fps": 8,
    },
}
...
class ErgonomiaVideoProcessor:
    ...
    def __init__(self, config):
        ...
        self.analysis_fps = max(4.0, float(self.cfg.get("analysis_fps",10)))
    ...
    def reset(self, clear_files=False):
        with self.lock:
            ...
            now0 = time.monotonic()
            self.last_tick = now0
            self.last_analysis_tick = now0
            self.last_analysis_run = 0.0
            self.total_time = 0.0
    ...
    def _capture_evidence(self, image, factor, value, values, now, lm=None, corners=None, ids=None):
        ...
        # Só aqui fazemos blur + esqueleto. O preview ao vivo permanece local e limpo.
        shot=image.copy()
        if lm is not None:
            try:
                self._blur_face_region(shot,lm)
                self._draw_skeleton(shot,lm)
            except Exception:
                pass
        if corners is not None and ids is not None:
            try:
                self._draw_markers(shot,corners,ids)
            except Exception:
                pass
    ...
    def _process_evidence(self, image, active_map, values, now, lm=None, corners=None, ids=None):
        severity={
        ...
        for factor,is_active in active_map.items():
            ...
            if capture:
                self._capture_evidence(
                    image,factor,val,values,now,
                    lm=lm,corners=corners,ids=ids
                )
    ...
    def recv(self, frame):
        ...
        # FPS da câmera recebido pelo servidor.
        ...
        # A câmera pode transmitir 20-24 FPS, mas a ergonomia é inferida em 8-10 FPS.
        # Frames intermediários não entram no MediaPipe, evitando fila/backpressure.
        if (now-self.last_analysis_run) < self.analysis_interval:
            return frame

        analysis_dt=now-self.last_analysis_tick
        self.last_analysis_tick=now
        self.last_analysis_run=now
        dt=clamp(analysis_dt,0.0,0.40)
        ...
        if lm is None:
            ...
            # SENDONLY: o navegador já mostra o preview local.
            # Não desenhamos nem recodificamos o frame no servidor.
            return frame
        ...
        # O preview ao vivo é local no tablet. Blur/esqueleto são aplicados
        # somente nas evidências salvas, evitando recodificação de vídeo.
        with self.lock:
            self.current=values
            if not valid_frame:
                self.invalid_time += dt
...
with st.sidebar:
    ...
    st.caption(
        "Use o tablet em paisagem. O preview é local; "
        "a IA recebe somente o necessário para análise, sem devolver vídeo processado."
    )
...
cfg={
    ...
    "analysis_fps":float(camera_profile["analysis_fps"]),
}
...
<div class="subtitle">MVP 04.2 — preview local no tablet + análise ergonômica assíncrona + evidências.</div>
</div></div>
""",unsafe_allow_html=True)
...
with tab_live:
    def render_camera_block():
        ...
        <div class="panel-sub">PREVIEW LOCAL: a imagem exibida vem direto da câmera do tablet. O Cloud recebe frames somente para análise ergonômica em segundo plano.</div></div>
        """,unsafe_allow_html=True)
        ...
        st.markdown(
            ...
            <div class="camera-spec">
              ...
              <div><div class="c-label">IA no Cloud</div><div class="c-value">{camera_profile['analysis_fps']} FPS · lado máx. {camera_profile['process_long_side']} px</div></div>
            </div>
            """,
            unsafe_allow_html=True,
        )
        ...
        st.success("Preview local ativo: o vídeo NÃO volta processado pelo Streamlit Cloud.")
        st.caption(
            "O esqueleto não é desenhado sobre o preview ao vivo para evitar atraso. "
            "RULA, REBA, IRE, tempos e evidências continuam sendo calculados normalmente. "
            "Se alterar câmera ou perfil, pare e inicie novamente."
        )
        return ctx_local
